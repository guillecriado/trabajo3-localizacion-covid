...
public class Poblacion {
	...
	public void addPersona(Persona persona) throws EmsDuplicatePersonException {
		try {
			findPersona(persona.getIdentidadPersona().getDocumento());
	...
	public void printPoblacion() {
		...
		for (int i = 0; i < lista.size(); i++) {
			...
			// Documento
			System.out.printf("%d;%s;", i, lista.get(i).getIdentidadPersona().getDocumento());
			// nombre y apellidos
			System.out.printf("%s,%s;", lista.get(i).getIdentidadPersona().getApellidos(),
					lista.get(i).getIdentidadPersona().getNombre());
			// correo electrónico
	...
	public String toString() {
		...
		for (int i = 0; i < lista.size(); i++) {
			FechaHora fecha = lista.get(i).getFechaNacimiento();
			// Documento
			cadena += String.format("%s;", lista.get(i).getIdentidadPersona().getDocumento());
			// nombre y apellidos
			cadena += String.format("%s,%s;", lista.get(i).getIdentidadPersona().getApellidos(),
					lista.get(i).getIdentidadPersona().getNombre());
			// correo electrónico
